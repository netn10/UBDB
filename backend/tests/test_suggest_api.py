...
def test_suggest_returns_results_shape(client):
    # conftest card oracle-1 has franchise "Special Guests" (no colors/keywords
    # in the fixture), so match on the franchise whole-phrase signal.
    ...
    assert any("special guests" in w.lower() for w in body["results"][0]["why"])
